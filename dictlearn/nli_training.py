...
def train_snli_model(new_training_job, config, save_path, params, fast_start, fuel_server, seed, model='simple'):
    ...
    if c['layout'] == 'snli':
        validation = DataStreamMonitoring(
            monitored_vars,
            data.get_stream('valid', batch_size=50, seed=seed),
            before_training=not fast_start,
            on_resumption=True,
            after_training=True,
        ...
        validation = DataStreamMonitoring(
            monitored_vars,
            data.get_stream('valid_matched', batch_size=50, seed=seed),
            every_n_batches=c['mon_freq_valid'],
            on_resumption=True,
            after_training=True,
        ...
        validation_mismatched = DataStreamMonitoring(
            monitored_vars,
            data.get_stream('valid_mismatched', batch_size=50, seed=seed),
            every_n_batches=c['mon_freq_valid'],
            before_training=not fast_start,
            on_resumption=True,
            after_training=True,
